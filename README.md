...
├─attacks                           # 对抗样本相关
│      base_attack.py                   # 随机噪声
│      data_label_map.dat               # 测试集中图片和标签的对应关系
...
事实上，这个方法一般用于白盒攻击。但只要适当控制epochs，就可以使用该算法进行无目标的黑盒攻击。通过实验，我发现，绝大多数样本在10次迭代内就会使模型出现误判。所以只需将epochs置为10，就可以对生成大部分有效的对抗样本。

同时，这个算法存在一个缺陷（有可能是我实现的问题），有时候会返回原图，即两张图片的`SSIM = 1`；在这个时候需要将算法进行退化，对图片使用iFGSM再次进行处理，根据经验，此时一般进行5次迭代即可得到有效的对抗样本。

但是因为连模型都未知，所以这个算法只能用来预训练一批数据，在使用时进行查询。如果传入的数据查询不到，则需要现场生成；这时算法再次退化，退化为图像合成。该算法基于以下两个假设：
...
[12] Su J, Vargas D V, Sakurai K. One pixel attack for fooling deep neural networks[J]. IEEE Transactions on Evolutionary Computation, 2019. 